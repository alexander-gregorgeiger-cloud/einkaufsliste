      ...
      <form onSubmit={handleAddItem} className="sticky top-0 z-10 -mx-4 px-4 pt-1 pb-3 bg-[#f0fdf4]/95 backdrop-blur-sm">
        <div className="flex gap-2">
          <input
            ...
            className="flex-1 px-4 py-3 bg-white border border-slate-200 rounded-xl text-2xl focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent shadow-sm"
          />
          <button
            type="submit"
              ...
              <div className="space-y-1.5">
                {checkedItems.map(item => (
                  ...
                  >
                    ...
                    <span className="flex-1 min-w-0 text-slate-400 text-2xl line-through truncate">{item.name}</span>
                    <button
                      onClick={() => deleteItem(item.id)}
